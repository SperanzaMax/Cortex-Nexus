...
from src.asombro import get_ente_experimental_prompt, ENTE_CONTROL_PROMPT, linguistic_profile
...
def clean_response(text: str) -> str:
    ...
    # Menciones de asombro
    text = re.sub(
        r'(mi (nivel de )?asombro|estado de asombro|'
        r'asombro: \d+\.\d+|'
        r'me siento asombrad[oa]|'
        r'estoy asombrad[oa]|'
        r'siento (una|cierta) expansión cognitiva|'
        r'tengo facilidad para conectar ideas)',
        '', text, flags=re.IGNORECASE
    )
...
# --- Core Cycle ---
async def run_cycle(client, config, tau, asombro, bloque_label, recent_questions):
    ...
    roles = [("exp", get_ente_experimental_prompt(asombro)), ("ctl", ENTE_CONTROL_PROMPT)]
    ...
    # 4. Cálculo de Métricas y Análisis Lingüístico
    s_exp = scores['exp']
    s_ctl = scores['ctl']
    ...
    # Perfiles lingüísticos para validación cruzada
    lp_exp = linguistic_profile(responses['exp'])
    lp_ctl = linguistic_profile(responses['ctl'])
