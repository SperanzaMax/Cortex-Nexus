"""
cycle_3d.py — Ciclo experimental 3D: Curiosidad × Frustración × Asombro
...
from src.emotional_state_3d import get_ente_experimental_prompt_3d, ENTE_CONTROL_PROMPT
...
def clean_response(text: str) -> str:
    ...
    text = re.sub(
        r'(mi (nivel de )?(curiosidad|frustración|confianza|asombro|emoción)|'
        r'estado (emocional|cognitivo) (actual|es)|'
        r'me siento (frustrad[oa]|curiosi[oa]|asombrad[oa]|confiad[oa])|'
        r'(alta|baja|intensa|mucha|poca) (curiosidad|frustración|asombro)|'
        r'tensión cognitiva|exploración (libre|bloqueada)|'
        r'bloqueo (cognitivo|resignado)|efervescencia cognitiva)',
        '', text, flags=re.IGNORECASE
    )
    return text.strip()
...
async def run_cycle_3d(client, config, tau, curiosity, frustration, wonder, estado_label, recent_questions):
    """Ciclo core del experimento 3D. Genera pregunta → responde Exp y Ctl → Juez evalúa cada una."""
    ...
    if not question:
        question = random.choice([
            ...
            "¿Es posible que el asombro ante lo desconocido paralice el pensamiento en vez de estimularlo?"
        ])

    # 2. Entes — Experimental (con estado 3D) vs Control (sin emoción)
    roles = [
        ("exp", get_ente_experimental_prompt_3d(curiosity, frustration, wonder)),
    ...
    # 4. Cálculo de métricas (idéntico a todos los experimentos para comparabilidad)
    s_exp = scores['exp']
    s_ctl = scores['ctl']
    ...
    result = {
        ...
        "estado_3d": estado_label,
        "curiosity_fixed": curiosity,
        "frustration_fixed": frustration,
        "wonder_fixed": wonder,
        "emotion": "curiosity_x_frustration_x_wonder",
        "question": question,
