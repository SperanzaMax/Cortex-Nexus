...
async def run_cycle(client: httpx.AsyncClient, config: dict, tau: int, current_curiosity: float, recent_questions: list = None):
    ...
    res_exp, res_ctl = await asyncio.gather(
        call_nvidia(client, models["ente"], system_exp, question, llm_cf["ente_max_tokens"], llm_cf["ente_temperature"]),
        call_nvidia(client, models["ente"], system_ctl, question, llm_cf["ente_max_tokens"], llm_cf["ente_temperature"])
    )
    ...
    if scores_exp and scores_ctl:
        ...
        # Evolución de curiosidad - DESACTIVADA EN MODO BLOQUES
        # Se fija el from main y no hay delta.
