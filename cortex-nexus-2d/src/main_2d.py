"""
main_2d.py — Motor principal del experimento 2D: Curiosidad × Frustración
Cortex-Nexus · 2026-04-15 | Maximiliano Rodrigo Speranza

Uso:
    python -m src.main_2d big
    python -m src.main_2d small
"""
import asyncio
import json
import logging
...
from src.cycle_2d import run_cycle_2d
...
async def verify_models(config: dict) -> bool:
    ...
    # NVIDIA: log informativo, no bloqueante (puede tener 429 temporales)
...
async def run_experiment_2d(config: dict, output_file: str):
    logging.info(f"\n{'='*60}")
    logging.info(f"INICIANDO: {config['experiment_name']}")
    logging.info(f"Espacio 2D: {len(config['estados_2d'])} estados × 30 ciclos = 240 ciclos total")
    ...
    async with httpx.AsyncClient(timeout=90.0) as client:
        for estado_cfg in config['estados_2d']:
            ...
            idx = config['estados_2d'].index(estado_cfg)
            tau_base = sum(e['cycles'] for e in config['estados_2d'][:idx])

            logging.info(
                f"\n{'─'*50}\n"
                f"ESTADO: {label} | C={curiosity:.2f} F={frustration:.2f} | {n_cycles} ciclos\n"
            ...
            for i in range(n_cycles):
                ...
                try:
                    ...
                    logging.info(
                        ...
                        f"C={curiosity:.2f} F={frustration:.2f} | {dur:.0f}s"
                    )
    ...
    logging.info(f"\n{'='*60}")
    logging.info(f"EXPERIMENTO 2D COMPLETADO: {config['experiment_name']}")
    logging.info(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f"logs/motor_2d_{size}.log"),
            logging.StreamHandler()
        ]
    )

    config_file = f"src/config_2d_{size}.json"
    output_file = f"data/cycles_2d_{size}.jsonl"
    ...
    asyncio.run(run_experiment_2d(config, output_file))
